...
filename = 'data/Cd_strip_4_eald(2).csv'
data = pd.read_csv(filename)

#adding column names to the data
data.columns = ['Voltage (V)', 'Current (I/uA)']
curr = data['Current (I/uA)']
vlt = data['Voltage (V)']

#getting the values of each row
current = data['Current (I/uA)'].values
volts = data['Voltage (V)'].values

#creating the plotly figure
fig = go.Figure()
#adding a trace of the current vs voltage
fig.add_trace(go.Scatter(x=volts, y=current, mode='lines', name='data', line=dict(color='blue', width=1)))
...
volts = volts.reshape(-1, 1) #reshaping voltage data from 1D array to 2D array

scaler = StandardScaler().fit(volts) #scales the data so it has a mean of 0 and a sd of 1. the fit method calculates the mean and sd
volts = scaler.transform(volts) #applies the scaling to the volts data. trasnofrm standardises the data by substracting the mean of the data and dividing by the sd
for degree in range(1, 150):
    ...
    reg = LinearRegression().fit(X, current) #fit trains the model on the input data X and the output variable current.
    # LinearRegression fits a linear regression model to the data X (the input features) and y (the output features, current).

    r2 = r2_score(current, reg.predict(X)) #calculates the coefficient of determination (r^2). takes the dependent variable, current,
    # and the predicted values of dependent variable obtained from the regression
    results = results.append({'degree order': degree, 'R^2 score': r2}, ignore_index=True) #appends the results to the dataframe
...
powers = poly.powers_
coef = np.polyfit(X[:,0], current, best_degree) #calculates the coefficients of the polynomial of degree best_degree
...
reg = LinearRegression().fit(X, current)
fig.add_trace(go.Scatter(x=vlt, y=reg.predict(X), mode='lines', name='regression', line=dict(color='red', width=1)))
fig.update_layout(title='Polynomial Regression of degree {} (R2={:.6f})'.format(best_degree, max_r2), xaxis_title='Voltage (V)', yaxis_title='Current (I/uA)')
...
idx = data.loc[data['Voltage (V)'] == value].index[0] #determines the index of the x coordinate in the data
idx_2 = data.loc[data['Voltage (V)'] == value_2].index[0]

x_0 = vlt.iloc[idx] #extracts the exact x coordinate from the data
y_0 = curr.iloc[idx]

x_1 = vlt.iloc[idx_2]
y_1 = curr.iloc[idx_2]

y_1 = reg.predict(poly.fit_transform(scaler.transform(np.array(value).reshape(-1, 1)))) #predicting the y value for the user-provided x coordinate using the previously trained model
...
fig.add_trace(go.Scatter(x=x, y=y, mode='lines', name='Line between selected Potentials', line=dict(color='green', width=1)))
fig.update_layout(title='Polynomial Regression of degree {} (R2={:.6f})'.format(best_degree, max_r2), xaxis_title='Voltage (V)', yaxis_title='Current (I/uA)')
fig.update_layout(legend=dict(yanchor="top",y=0.99,xanchor="left",x=0.0))
...
x = np.linspace(x_0, x_1, len(y_pred)) #creates an array, x, containing a sequence of evenly spaced numbers between x_0 and x_1 with the same length as y_pred
def line(x, m, b):
    return [m*i + b for i in x] #defines a function line that takes x, m and b as arguments and returns an array where each element is a value of the equation
            # for each element i in the array x
y_line = line(x, m, b) #calls the line function and assigns the result to y_line. produces an array that represents the line of best fit
...
area = fixed_quad(lambda x: abs(np.interp(x.flatten(), vlt, y_pred_flat)), x_0, x_1, n=1000)
